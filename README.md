# AI-agent kit `v7.0.0-beta`

AI-agent configuration kit for [OpenCode](https://opencode.ai) and [Claude Code](https://claude.com/product/claude-code). Drops a complete, **deliberately small** agent team into your project — up to 9 agents in legacy roster (unchanged since v5) or 5 agents in consolidated roster (Architect, Verifier, CodeWriter, BugFixer, Main; opt-in via manifest flags), with hard slice caps, a mandatory diff-review gate, scope-drift detection, a frozen-vs-mutable spec/plan split, per-step commits, vertical-slice gate, runbook reports, clean-session-per-step automation, defect-feedback at 5.6, an autonomous `sleep mode` for unattended runs, non-destructive `/kit-revert-step`, token-budget slice cap, mandatory ground-truth artefact gate at 5.6, `defect_origin` and `gate_signal_ratio` telemetry, risk-based pipeline triage (trivial/standard/critical lanes opt-in), and **(new in v7.0.0-beta)** **agent consolidation infrastructure**: @Architect (= Analyst + Designer in one dispatch) and @Verifier (= TestKeeper + Reviewer + DoDGate + TraceabilityChecker mode-driven). Both consolidations are per-merger opt-in flags so PO can A/B test against gates.csv signal_ratio before committing.

v7.0.0-beta ships both legacy and consolidated agent bodies side-by-side. With `agents.consolidated_architect: false` and `agents.consolidated_verifier: false` (defaults) the pipeline behaves exactly like v7.0.0-alpha. Each merger flips independently — PO validates one consolidation via 15–30 tasks of telemetry, then enables the next. v7.0.0-stable will remove the legacy bodies once aggregate data across the user community validates the mergers.
