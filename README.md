...
AI-agent configuration kit for [OpenCode](https://opencode.ai) and [Claude Code](https://claude.com/product/claude-code). Drops a complete, **deliberately small** agent team into your project — 9 agents (unchanged since v5), with hard slice caps, a mandatory diff-review gate, scope-drift detection, a frozen-vs-mutable spec/plan split, per-step commits, vertical-slice gate, runbook reports, clean-session-per-step automation, defect-feedback at 5.6, an autonomous `sleep mode` for unattended runs, non-destructive `/kit-revert-step` (no harness double-prompt), token-budget slice cap, mandatory ground-truth artefact gate at 5.6, `defect_origin` telemetry, and **(new in v6.3)** **`gate_signal_ratio` telemetry** — every gate verdict is logged to `evals/runs/<version>/gates.csv` and `/kit-status` highlights gates with rolling signal_ratio below threshold as deprecation candidates.

v6.3 is a telemetry-only release — no agent rewrites, no pipeline branches. Together with v6.2's defects.csv it gives the kit a self-cleaning gate-evaluation mechanism: the data side of v7's data-driven agent consolidation. Opt-in by `evals/runs/<kit_version>/` directory presence; without that directory, v6.3 is invisible.

v6.2 added the research-aligned foundation: P0 fix `/kit-revert-step`, P17 token-budget cap, P18 ground-truth gate, P19 defect_origin telemetry. The full v7 plan — 3 risk lanes (trivial/standard/critical) and 9→5 agent consolidation — is documented in [PIPELINE_V7_PROPOSAL.md](PIPELINE_V7_PROPOSAL.md). v6.2 + v6.3 land the highest-ROI subset that does not require pipeline restructure.
