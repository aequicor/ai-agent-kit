# AI-agent kit `v7.0.0-alpha`

AI-agent configuration kit for [OpenCode](https://opencode.ai) and [Claude Code](https://claude.com/product/claude-code). Drops a complete, **deliberately small** agent team into your project — 9 agents (unchanged since v5; v7.0.0-beta will consolidate to 5 based on accumulated telemetry data), with hard slice caps, a mandatory diff-review gate, scope-drift detection, a frozen-vs-mutable spec/plan split, per-step commits, vertical-slice gate, runbook reports, clean-session-per-step automation, defect-feedback at 5.6, an autonomous `sleep mode` for unattended runs, non-destructive `/kit-revert-step`, token-budget slice cap, mandatory ground-truth artefact gate at 5.6, `defect_origin` and `gate_signal_ratio` telemetry, and **(new in v7.0.0-alpha)** **risk-based pipeline triage** with three lanes: `trivial` (≤1 file, ≤30 lines — collapsed pipeline saves ~70% tokens), `standard` (current v6.3 pipeline), `critical` (security/migration/external API — adversarial review + mutation-sample + sleep forbidden).

v7.0.0-alpha is **opt-in by manifest flag**. With `lanes.enabled: false` (default) the pipeline behaves exactly like v6.3 — the v7 path activates when PO flips the flag. v7.0.0-beta will consolidate agents 9→5 (Architect = Analyst+Designer; Verifier = TestKeeper+Reviewer+DoDGate+TraceabilityChecker) based on data accumulated by v6.3's `gate_signal_ratio` telemetry. v7.0.0-stable will make lanes mandatory and ground-truth mandatory in critical.
