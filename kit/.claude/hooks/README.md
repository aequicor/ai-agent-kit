...
| `pre-tool-bash-guard.mjs` | `PreToolUse` (matcher: `Bash`) | (a) Hard-denies `git push --force` to `main/master/prod` regardless of permission table; (b) Hard-denies writes to credential files (`.env*`, `~/.ssh`, `~/.aws`); (c) Pre-commit sync check — if `src/**` is staged without any test, `test-cases.md`, or `feature.md` update, asks PO before proceeding. |
...
- The model still has to mark steps in `feature.md § Implementation plan` (prompt rule).
