...
Shared context (project, modules, file-access matrix, tool naming, MCP/skills) — `{{HOST_DIR}}/_shared.md`.
...
4. (cache write — MANDATORY after 2 or 3) knowledge-my-app_write_guideline
   → {{VAULT_PATH}}/guidelines/libs/<lib>-<version>.md with frontmatter (lib, version, source, date)
...
1a. If `{{VAULT_PATH}}/reference/[module]/spec/[feature]-test-plan.md` exists → read the **Unit Tests** and **Integration Tests** sections.
...
```
1. Read the relevant rows of {{VAULT_PATH}}/reference/[module]/test-cases/[feature]-test-cases.md.
...
2. For each owned TC, write a test in the mirrored structure under src/test/ (or equivalent).
   Test naming convention follows {{HOST_DIR}}/skills/spec-to-code-trace/SKILL.md
   so @QA IMPL FINAL can attach `(impl: <path>)` references to TC rows automatically.
3. Run: `{{TEST_COMMAND_TEMPLATE}}`. Every new test MUST FAIL right now —
...
3. If success → run `{{TEST_COMMAND_TEMPLATE}}` — the tests added in Step 2 must move from FAIL → PASS
...
After successful build: `{{LINT_COMMAND}}`
